...
const MOCK = [
  ...
  { id:"DOS-2024-002", client:"Emilie Rousseau", email:"emilie@example.com", phone:"06 98 76 54 32", address:"5 avenue Victor Hugo, 69001 Lyon", dp_number:"DP 069 011 24 00042", works:[{type:"Panneaux Solaires",dossier_type:"Recuperation TVA"}], status:"valide", assignee:"Nada", created:"2024-10-08", updated:"2024-11-30", paid:true, amount:800, docs:[{name:"Facture_Panneaux.pdf",size:"180 KB",date:"2024-10-10"}], notes:[{author:"Nada",date:"2024-11-30",text:"Dossier valide, TVA recuperee."}], client_access:false },
  { id:"DOS-2024-003", client:"Jean-Pierre Moreau", email:"jp@example.com", phone:"07 11 22 33 44", address:"28 chemin du Moulin, 13300 Salon-de-Provence", dp_number:"DP 013 055 24 00078", works:[{type:"Menuiseries Exterieures",dossier_type:"CONSUEL"},{type:"Systeme Solaire Combine",dossier_type:"Demande Prealable Raccordement"}], status:"nouveau", assignee:"Jimmy", created:"2024-12-05", updated:"2024-12-05", paid:false, amount:2500, docs:[], notes:[], client_access:true },
  { id:"DOS-2024-004", client:"Isabelle Bernard", email:"isa@example.com", phone:"06 55 44 33 22", address:"3 rue du Port, 44000 Nantes", dp_number:"DP 044 109 24 00015", works:[{type:"ITE",dossier_type:"Recuperation TVA"}], status:"en_attente", assignee:"Farah", created:"2024-09-20", updated:"2024-11-10", paid:false, amount:950, docs:[{name:"Plan_isolation.pdf",size:"520 KB",date:"2024-09-22"}], notes:[{author:"Farah",date:"2024-11-10",text:"En attente du retour de l administration."}], client_access:false },
  { id:"DOS-2024-005", client:"Thomas Lefebvre", email:"thomas@example.com", phone:"06 77 88 99 00", address:"17 bd de la Republique, 33000 Bordeaux", dp_number:"DP 033 063 24 00091", works:[{type:"PAC",dossier_type:"Demande Prealable Raccordement"},{type:"PAC",dossier_type:"CONSUEL"}], status:"termine", assignee:"Harry", created:"2024-08-01", updated:"2024-11-25", paid:true, amount:1800, docs:[{name:"Attestation_PAC.pdf",size:"300 KB",date:"2024-11-25"}], notes:[{author:"Harry",date:"2024-11-25",text:"Dossier complet et termine."}], client_access:false },
];
...
.doc-item { display: flex; align-items: center; gap: 10px; padding: 11px 14px; background: #f8f9fc; border: 1px solid #e2e6f0; border-radius: 8px; margin-bottom: 7px; }
    ...
    <div className="toast-c">
      {toasts.map(t => (
        <div key={t.id} className={"toast " + t.type}>
          ...
          <button onClick={() => rm(t.id)} style={{background:"none",border:"none",color:"#9ca3af",cursor:"pointer",display:"flex"}}>
            <SI n="x" s={13}/>
          </button>
        </div>
      ))}
    ...
    <div className="login-page">
      <div className="login-card">
        <div style={{textAlign:"center", marginBottom:28}}>
          <div style={{width:52,height:52,background:"#4f46e5",borderRadius:14,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 14px",fontSize:24}}>
            ☀️
          </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:13}}>
          <div className="fg">
            ...
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="votre@email.fr" onKeyDown={e => e.key==="Enter" && go()}/>
          </div>
          <div className="fg">
            <label>Mot de passe</label>
            <input type="password" value={pw} onChange={e => setPw(e.target.value)} placeholder="••••••••" onKeyDown={e => e.key==="Enter" && go()}/>
          </div>
          <button className="btn btn-p" style={{width:"100%",justifyContent:"center",padding:12,marginTop:4}} onClick={go}>
            <SI n="lock" s={15}/>Se connecter
          </button>
        </div>
  ...
  return (
    <div>
      <div className="stats-grid">
        {[
          {l:"Total dossiers", v:total, c:"#4f46e5"},
          {l:"En cours", v:enCours, c:"#d97706"},
          {l:"Termines", v:termines, c:"#059669"},
          {l:"Impayes", v:impayes, c:"#dc2626"},
        ].map((s,i) => (
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
        <div className="card">
          <h3 style={{fontSize:14,fontWeight:700,marginBottom:14,color:"#111827"}}>Repartition statuts</h3>
          {STATUSES.map(s => {
            const c = dossiers.filter(d => d.status === s.key).length;
            return (
              <div key={s.key} style={{display:"flex",alignItems:"center",gap:8,marginBottom:9}}>
                ...
                <div style={{flex:2,background:"#eef0f6",borderRadius:3,height:7,overflow:"hidden"}}>
                  <div style={{width: (total ? c/total*100 : 0) + "%", height:"100%",background:s.color,borderRadius:3}}/>
                </div>
      ...
      <div className="card">
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
          {WORK_TYPES.map(wt => {
            const c = dossiers.filter(d => d.works.some(w => w.type === wt)).length;
            return (
      ...
      <div className="modal">
        ...
        <div className="mbody">
          <div className="form-grid" style={{marginBottom:18}}>
            ...
            <div className="fg"><label>Numero DP</label><input value={f.dp_number} onChange={e => set("dp_number",e.target.value)} placeholder="DP 075 111 24 00001"/></div>
          ...
          <div className="form-grid" style={{marginTop:14}}>
            ...
            <div className="fg" style={{gap:10,justifyContent:"flex-end"}}>
              <label style={{display:"flex",alignItems:"center",gap:7,cursor:"pointer",fontSize:13,fontWeight:500}}>
                <input type="checkbox" checked={f.paid} onChange={e => set("paid",e.target.checked)}/>Paye
              </label>
              <label style={{display:"flex",alignItems:"center",gap:7,cursor:"pointer",fontSize:13,fontWeight:500}}>
                <input type="checkbox" checked={f.client_access} onChange={e => set("client_access",e.target.checked)}/>Acces client
              </label>
            </div>
          </div>
        </div>
...
function DossierDetail({dossier, onClose, onUpdate, currentUser, toast}){
  ...
  const save = (u) => {
    ...
    setD(nd); onUpdate(nd); toast("Mis a jour","s");
  };
  const addNote = () => {
    if(!note.trim()) return;
    save({notes:[...d.notes, {author:currentUser.name, date:new Date().toISOString().split("T")[0], text:note}]});
    setNote("");
  };
          ...
          {tab === "info" && (
            <div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18}}>
                <div>
                  {[["Email",d.email],["Telephone",d.phone],["Adresse",d.address],["N° DP",d.dp_number]].map(([l,v]) => (
                  ...
                  <div style={{marginBottom:14}}>
                    ...
                    {d.works.map((w,i) => (
                      <div key={i} style={{marginBottom:6}}>
                        ...
                        <span className="chip chip-d">{w.dossier_type}</span>
                      </div>
                    ))}
                  </div>
              ...
              {[...d.notes].reverse().map((n,i) => (
                <div className="note-item" key={i}>
                  <div style={{fontSize:11,color:"#9ca3af",marginBottom:5,display:"flex",gap:8}}>
                    <strong style={{color:"#4f46e5"}}>{n.author}</strong><span>{n.date}</span>
                  </div>
            ...
            <div>
              ...
              {currentUser.role === "superadmin" && (
                <div style={{display:"flex",gap:10}}>
                  <button className="btn btn-g" disabled={d.paid} style={{opacity:d.paid?.5:1}} onClick={() => {save({paid:true}); toast("Paiement valide - Super Admin notifie","s");}}>
                    <SI n="check" s={14}/>Marquer paye
                  </button>
                  <button className="btn btn-d" disabled={!d.paid} style={{opacity:!d.paid?.5:1}} onClick={() => save({paid:false})}>
                    Annuler
                  </button>
                </div>
              )}
    ...
    <div>
      ...
      <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:16}}>
        <div className="srch-wrap">
          <span className="srch-ico"><SI n="search" s={15}/></span>
          <input className="srch" placeholder="Nom, adresse, n° DP..." value={q} onChange={e => setQ(e.target.value)}/>
        </div>
        <select className="fsel" value={fSt} onChange={e => setFSt(e.target.value)}>
          <option value="all">Tous statuts</option>{STATUSES.map(s => <option key={s.key} value={s.key}>{s.label}</option>)}
        </select>
        <select className="fsel" value={fAs} onChange={e => setFAs(e.target.value)}>
          <option value="all">Tous responsables</option>{EMPLOYEES.map(e => <option key={e}>{e}</option>)}
        </select>
        <select className="fsel" value={fWk} onChange={e => setFWk(e.target.value)}>
          <option value="all">Tous travaux</option>{WORK_TYPES.map(t => <option key={t}>{t}</option>)}
        </select>
      </div>
      <div className="card" style={{padding:0,overflow:"hidden"}}>
        <div style={{overflowX:"auto"}}>
          <table>
            <thead>
              <tr><th>ID</th><th>Client</th><th>Adresse</th><th>Travaux</th><th>Responsable</th><th>Statut</th><th>Paiement</th><th></th></tr>
            </thead>
            <tbody>
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:10,marginBottom:18}}>
        <div className="srch-wrap">
          <span className="srch-ico"><SI n="search" s={15}/></span>
          <input className="srch" placeholder="Nom, email, adresse..." value={q} onChange={e => setQ(e.target.value)}/>
        </div>
      </div>
        ...
        {clients.map(c => (
          <div className="card" key={c.email}>
            <div style={{display:"flex",gap:11,marginBottom:12,alignItems:"flex-start"}}>
              ...
              <div style={{flex:1}}>
                <div style={{fontWeight:700,fontSize:15,color:"#111827"}}>{c.client}</div>
        ...
        {unpaid.map(d => (
          <div key={d.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:"1px solid #e2e6f0",flexWrap:"wrap"}}>
            <div style={{flex:1,minWidth:130}}>
              <div style={{fontWeight:600,color:"#111827"}}>{d.client}</div>
              <div style={{fontSize:11,color:"#9ca3af",marginTop:2}}>{d.id} · {d.assignee}</div>
            </div>
      ...
      <div className="card">
        ...
        {paid.map(d => (
          <div key={d.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:"1px solid #e2e6f0",flexWrap:"wrap"}}>
            <div style={{flex:1}}>
              <div style={{fontWeight:600,color:"#111827"}}>{d.client}</div>
              <div style={{fontSize:11,color:"#9ca3af"}}>{d.id}</div>
            </div>
  ...
  const handle = (files) => {
    ...
    r.onload = e => {
      try {
        ...
        toast(rows.length + " enregistrements detectes","i");
      } catch(err) { toast("Erreur de lecture","e"); }
    };
    r.readAsText(file);
  };
  const confirm = () => {
    const now = new Date().toISOString().split("T")[0];
    const nd = preview.map((row,i) => ({
      id: "IMP-" + Date.now() + "-" + i,
      client: row.client || row.nom || ("Client " + (i+1)),
      email: row.email || "", phone: row.phone || row.tel || "",
      address: row.address || row.adresse || "",
      dp_number: row.dp_number || "",
      works: [{type: row.type_travaux || "ITE", dossier_type: "Demande Prealable Raccordement"}],
      status: "nouveau", assignee: EMPLOYEES[0], paid: false,
      amount: Number(row.amount || 0),
      created: now, updated: now, docs: [], notes: [], client_access: false, client_token: null
    }));
    setDossiers(p => [...nd,...p]); setPreview([]);
    toast(nd.length + " dossiers importes","s");
  };
  return (
    <div>
      ...
      <div className="card" style={{marginBottom:16}}>
        ...
        <code style={{display:"block",background:"#f8f9fc",padding:12,borderRadius:8,fontSize:12,color:"#4f46e5",overflowX:"auto",border:"1px solid #e2e6f0"}}>
      ...
      <div className="app">
        ...
        <div className={"sb" + (sbOpen?" open":"")}>
          ...
          <div className="sb-user">
            ...
            <button style={{background:"none",border:"none",cursor:"pointer",padding:4,color:"#9ca3af",display:"flex"}} onClick={() => setUser(null)}>
              <SI n="logout" s={15}/>
            </button>
          </div>
        </div>
        <div className="main">
          <div className="topbar">
            ...
            <button className="bic" onClick={() => toast("Aucune nouvelle notification","i")}><SI n="bell" s={17} c="#374151"/></button>
